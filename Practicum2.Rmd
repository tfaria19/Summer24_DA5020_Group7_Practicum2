...
ggplot(bed_fact_df, aes(x = license_beds)) +
  ...
  labs(title = "Data Distribution - bed_fact.csv - License", 
       x = "# of License Beds", 
       y = "Frequency",
       caption = "Figure 1a. Histogram showing distrubtion of license bed counts") +
  theme_classic() +
  theme(
...
ggplot(pivot_min1_license_beds_visual_df, aes(reorder(hospital_name, -bed_count), bed_count, fill = bed_type)) +
  ...
  caption = "Figure 4C: Minimum 1 ICU and SICU license bed per hospital"
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_classic() +
  theme(
...
ggplot(pivot_min1_census_beds_visual_df, aes(reorder(hospital_name, -bed_count), bed_count, fill = bed_type)) +
  ...
  caption = "Figure 4C: Minimum 1 ICU and SICU census bed per hospital"
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_classic() +
  theme(
...
We visualized the distribution of ICU and SICU bed counts with stacked bar charts for license, census, and staffed beds in Figure 4A, 4B, and 4C, respectively. We recommend considering hospitals that not only have at least 1 of each type of bed, but an equal distribution of both types to maximize cost-efficiency and avoid bed shortages. Because of this, we believe hospitals with high total bed counts are not necessarily better candidates if they only have more of one bed type. For example, the total bed count at Shands Hospital at The University of Florida in Figures 4A, 4B, and 4C shows a majority SICU beds and small portion of ICU beds. We would not recommend this hospital due to the potential ICU bed shortages. However, we would recommend Jackson Memorial Hospital, which has less overall beds compared to Shands but has an equal distribution of both types.
