...
ggplot(bed_fact_df, aes(x = license_beds)) +
  ...
  labs(title = "Data Distribution - bed_fact.csv - License", x = "# of License Beds", y = "Frequency") +
  theme_minimal()
...
ggplot(bed_fact_df, aes(x = census_beds)) +
  ...
  scale_x_continuous(limits = c(-1, 3000)) +
  scale_y_continuous(limits = c(-1, 3000)) +
  labs(title = "Data Distribution - bed_fact.csv - Census", x = "# of Census Beds", y = "Frequency") +
  theme_minimal()
...
ggplot(bed_fact_df, aes(x = staffed_beds)) +
  ...
  scale_x_continuous(limits = c(-1, 3000)) +
  scale_y_continuous(limits = c(-1, 3000)) +
  labs(title = "Data Distribution - bed_fact.csv - Staffed", x = "# of Staffed Beds", y = "Frequency") +
  theme_minimal()
...
### License Beds (ICU and SICU Separated)

```{r, license_beds_separate}

license_beds_icu <- "SELECT business_name AS hospital_name, 
SUM(license_beds) AS total_license_beds_icu
FROM combined_table
WHERE bed_id = 4
GROUP BY hospital_name
ORDER BY total_license_beds_icu DESC
LIMIT 10"

license_beds_icu_df <- dbGetQuery(p2_connection, license_beds_icu)
license_beds_icu_df

license_beds_sicu <- "SELECT business_name AS hospital_name, 
SUM(license_beds) AS total_license_beds_sicu 
FROM combined_table
WHERE bed_id = 15
GROUP BY hospital_name
ORDER BY total_license_beds_sicu DESC
LIMIT 10"

license_beds_sicu_df <- dbGetQuery(p2_connection, license_beds_sicu)
license_beds_sicu_df
```

<<<<<<< HEAD
```{r}
total_license_beds_icu_sicu <- "SELECT business_name AS hospital_name, SUM(license_beds) AS total_license_icu_sicu_beds
FROM combined_table
WHERE bed_id IN (4, 15)
GROUP BY hospital_name
ORDER BY total_license_icu_sicu_beds DESC
LIMIT 10"


total_license_beds_icu_sicu <- dbGetQuery(p2_connection, total_license_beds_icu_sicu)

print(total_license_beds_icu_sicu)
```
=======
### Census Beds (ICU and SICU Separated)
>>>>>>> 99f06c411af86ac2ebc84e821e633b329b05e0e1

```{r, census_beds_separate}
# Sairah

census_beds_icu <- "SELECT business_name AS hospital_name, 
SUM(census_beds) AS total_census_beds_icu
FROM combined_table
WHERE bed_id = 4
GROUP BY hospital_name
ORDER BY total_census_beds_icu DESC
LIMIT 10"

census_beds_icu_df <- dbGetQuery(p2_connection, census_beds_icu)
census_beds_icu_df

ggplot(census_beds_icu_df, aes(x = hospital_name, y = total_census_beds_icu)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(
  x = "Hospital Name",
  y = "Total Census Beds (ICU)",
...
```{r}
census_beds_sicu <- "SELECT business_name AS hospital_name, 
SUM(census_beds) AS total_census_beds_sicu
FROM combined_table
WHERE bed_id = 15
GROUP BY hospital_name
ORDER BY total_census_beds_sicu DESC
LIMIT 10"

census_beds_sicu_df <- dbGetQuery(p2_connection, census_beds_sicu)
census_beds_sicu_df

ggplot(census_beds_sicu_df, aes(x = hospital_name, y = total_census_beds_sicu)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(
  x = "Hospital Name",
  y = "Total Census Beds (SICU)",
...
```
```{r}
total_census_beds_icu_sicu <- "SELECT business_name AS hospital_name, SUM(census_beds) AS total_census_icu_sicu_beds
FROM combined_table
WHERE bed_id IN (4, 15)
GROUP BY hospital_name
ORDER BY total_census_icu_sicu_beds DESC
LIMIT 10"


total_census_beds_icu_sicu <- dbGetQuery(p2_connection, total_census_beds_icu_sicu)

print(total_census_beds_icu_sicu)

```

### Staffed Beds (ICU and SICU Separated)
...
ggplot(pivot_staffed_beds_df, aes(reorder(hospital_name, -bed_count), bed_count, fill = bed_type)) +
  ...
  y = "Staffed Bed Count",
  fill = "Bed",
  title = "Hospitals with the highest combined ICU and SICU staffed bed
  counts",
  caption = "Figure 3C: ICU and SICU staffed beds combined with no minimum required count"
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_classic() +
  theme(
  ...
  plot.title = element_text(hjust = 0.8, face = "bold", margin = margin(b = 20)),
...
license_beds_combined <- "SELECT business_name AS hospital_name, 
SUM(license_beds) AS total_license_beds_combined
FROM combined_table
...
ORDER BY total_license_beds_combined DESC
LIMIT 10;"

license_beds_combined_df <- dbGetQuery(p2_connection, license_beds_combined) %>%
  as_tibble()
license_beds_combined_df
...
census_beds_combined_visual <- "SELECT business_name AS hospital_name,
...
census_beds_combined_visual_df <- dbGetQuery(p2_connection, census_beds_combined_visual) %>%

census_beds_combined <- "SELECT business_name AS hospital_name, 
SUM(census_beds) AS total_census_beds_combined
FROM combined_table
WHERE (bed_id = 4 OR bed_id = 15)
GROUP BY hospital_name
HAVING COUNT(DISTINCT CASE WHEN bed_id = 4 THEN 1 END) > 0
AND COUNT(DISTINCT CASE WHEN bed_id = 15 THEN 1 END) > 0
ORDER BY total_census_beds_combined DESC
LIMIT 10;"

census_beds_combined_df <- dbGetQuery(p2_connection, census_beds_combined) %>%
  as_tibble()

census_beds_combined_df
```
...
census_beds_combined_df <- dbGetQuery(p2_connection, census_beds_combined_visual) %>%
  as_tibble %>%
  slice(1:10) %>%
  rename(ICU = ICU_beds,
         SICU = SICU_beds)

census_beds_combined_visual_df

pivot_combined_census_beds_visual_df <- census_beds_combined_visual_df %>%
  pivot_longer(cols = c(ICU, SICU), names_to = "bed_type", values_to = "bed_count")
```

```{r}
# Bar Chart for ICU and SICU Census Beds for Top 10 Hospitals 
ggplot(pivot_combined_census_beds_df, aes(x = reorder(hospital_name, -bed_count), y = bed_count, fill = bed_type)) +
  geom_bar(stat = "identity") +
  labs(
    x = "Hospital Name",
    y = "Census Bed Count",
    fill = "Bed",
    title = "Hospitals with the Highest Combined ICU and SICU Census Bed Counts"
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
...
SUM(census_beds) AS total_staffed_beds_combined
FROM combined_table
WHERE (bed_id = 4 OR bed_id = 15)
GROUP BY hospital_name
HAVING COUNT(DISTINCT CASE WHEN bed_id = 4 THEN 1 END) > 0
AND COUNT(DISTINCT CASE WHEN bed_id = 15 THEN 1 END) > 0
ORDER BY total_staffed_beds_combined DESC
LIMIT 10;"

staffed_beds_combined_df <- dbGetQuery(p2_connection, staffed_beds_combined)
staffed_beds_combined_df
```

```{r staffed_beds_combined_visual}
...
ggplot(pivot_combined_staffed_beds_visual_df, aes(reorder(hospital_name, -bed_count), bed_count, fill = bed_type)) +
  ...
  y = "Staffed Bed Count",
  fill = "Bed",
  title = "Hospitals with Highest Combined Total ICU and SICU Staffed Beds",
  caption = "Figure 4C: Minimum 1 ICU and SICU staffed bed per hospital"
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
  theme_classic() +
  theme(
...
```{r}
### Rough draft query to use for final recommendation ###
...
```{r all beds visual}
### Rough draft visual to use for final recommendation ###
ggplot(pivot_all_beds_visual_df, aes(reorder(hospital_name, -bed_count), bed_count, fill = interaction(bed_type, icu_sicu))) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ icu_sicu, ncol = 1, scales = "free_x") +
  labs(
    x = "Hospital Name",
    y = "Bed Count",
    fill = "Bed Type",
    title = "Bed Counts by Hospital and Bed Type (ICU and SICU)"
  ) +
  theme_minimal() +
  theme(
...
We recommend considering hospitals that not only have at least 1 of each type of bed, but an equal distribution of both types. To visualize the distribution of beds  
