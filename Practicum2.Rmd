...
# Sairah
census_beds_separate <- "SELECT business_table.business_name AS hospital_name, SUM(bed_fact_table.census_beds) AS total_icu_sicu_census_beds
FROM business_table
JOIN bed_fact_table ON business_table.ims_org_id = bed_fact_table.ims_org_id 
...
dbGetQuery(p2_connection, census_beds_separate)

census_beds_separate <- dbGetQuery(p2_connection, census_beds_separate)

library(ggplot2)

ggplot(census_beds_separate, aes(x = hospital_name, y = total_icu_sicu_census_beds)) + geom_bar(stat = "identity", fill = "light blue") + theme_minimal() + coord_flip() + labs(x = "Hospital Name", y = "Total ICU and SICU Census Beds")
