...
```{r}
get_staffed_beds_separate <- "SELECT business_name AS hospital_name,
...
separate_staffed_beds_df <- dbGetQuery(p2_connection, get_staffed_beds_separate) %>%
  as_tibble() %>%
  slice(1:10)
separate_staffed_beds_df

pivot_staffed_beds_df <- separate_staffed_beds_df %>%
  pivot_longer(cols = c(ICU_beds, SICU_beds), names_to = "bed_type", values_to = "bed_count")
